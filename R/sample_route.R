...
#'   trips,
#'   abuse_ratio = 0.3, abuse_multiplier = 1.1
#' )
...
sample_route <- function(tripID, trips, r = NULL, timeBin_x_edges = NULL, rho = 0.31, abuse_ratio = 0, abuse_multiplier = 1) {
  ...
  filtered_time <- as.numeric(trips[trip %in% filtered_tripID, .(difftime(time[.N], time[1], units = "secs")), trip]$V1)
  filtered_length <- trips[trip %in% filtered_tripID, .(sum(length)), trip]$V1
  ...
  simulated_edge_num <- trips[, .(
    trip = 1:r,
    len = sample(filtered_edge_count, r, replace = TRUE)
  )]
  ...
  simulated_start_time <- sample(filtered_start_time, r, replace = TRUE)
