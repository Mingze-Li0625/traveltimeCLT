...
#' @export
predict.cov<-function(object, tripdata, starttime = Sys.time(),  n = 1000, logE = NULL,time_bins.fun = time_bins, ... ){
    ...
    if(grepl('HMM', object$model))
      predict.cov.HMM(object, tripdata, starttime, n, logE, time_bins = time_bins.fun, ...)
    else
      predict.cov.no_dependence(object, tripdata , starttime, n, logE, time_bins = time_bins.fun, ...)
}
...
#' {Woodard, D., Nogin, G., Koch, P., Racz, D., Goldszmidt, M., Horvitz, E., 2017.  Predicting travel time reliability using mobile phone GPS data.  Transportation Research Part C, 75, 30-44.}
#' @export
predict.cov.no_dependence <- function(object, tripdata, starttime, n = 1000, logE = NULL, time_bins = time_bins, ...) {
    el = list(...)
    obs = el$obs
    ...
    cov = logical(length(linkIds))
    q = quantile(tt, c(0.025, 0.975))
    cov[1] = obs[1] >= q[1] & obs[1] <= q[2]
...
#' {Woodard, D., Nogin, G., Koch, P., Racz, D., Goldszmidt, M., Horvitz, E., 2017.  Predicting travel time reliability using mobile phone GPS data.  Transportation Research Part C, 75, 30-44.}
#' @export
predict.cov.HMM <- function(object, tripdata, starttime, n, logE, time_bins = time_bins, ...) {
    el = list(...)
    obs = el$obs
    linkIds = tripdata$linkID # Contains IDs of all links for a given trip
    len = tripdata$length # Contains the length (in km) of each link in 'linkIds'
