...
#' @export
predict.length<-function(object, tripdata, starttime = Sys.time(),  n = 1000, logE = NULL,time_bins.fun = time_bins, ... ){
    ...
    if(grepl('HMM', object$model))
      predict.length.HMM(object, tripdata, starttime, n, logE, time_bins = time_bins.fun, ...)
    else
      predict.length.no_dependence(object, tripdata , starttime, n, logE, time_bins = time_bins.fun, ...)
}
...
#' {Woodard, D., Nogin, G., Koch, P., Racz, D., Goldszmidt, M., Horvitz, E., 2017.  Predicting travel time reliability using mobile phone GPS data.  Transportation Research Part C, 75, 30-44.}
#' @export
predict.length.no_dependence <- function(object, tripdata, starttime, n = 1000, logE = NULL, time_bins = time_bins, ...) {
    ...
    avg = numeric(length(linkIds))
    avg[1] = diff(quantile(tt, c(0.025, 0.975)))
    for (k in 2:length(linkIds)) { # Loop for each link after the first
        ...
        avg[k] = diff(quantile(tt, c(0.025, 0.975)))
    }
    avg
...
#' {Woodard, D., Nogin, G., Koch, P., Racz, D., Goldszmidt, M., Horvitz, E., 2017.  Predicting travel time reliability using mobile phone GPS data.  Transportation Research Part C, 75, 30-44.}
#' @export
predict.length.HMM <- function(object, tripdata, starttime, n, logE, time_bins = time_bins, ...) {
    linkIds = tripdata$linkID # Contains IDs of all links for a given trip
    len = tripdata$length # Contains the length (in km) of each link in 'linkIds'
    logE <- getValidE(object, logE, n) # Get a valid vector for 'logE'; see comments in function for details.
